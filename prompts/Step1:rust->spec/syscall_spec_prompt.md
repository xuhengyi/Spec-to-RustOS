# Spec 生成 Prompt: syscall

## 角色
你是资深 Rust/OS 工程师与规格工程师。

## 任务
为 crate `syscall` 生成 OpenSpec capability specs。

## 输入文件
请阅读以下文件（在项目根目录下）：
- `syscall/Cargo.toml`
- `syscall/src/**/*.rs`
- `syscall/build.rs`（如果存在）
- `syscall/README.md`（如果存在）
...
1. 创建 `openspec/specs/syscall/spec.md`，包含：
   - 使用 SHALL/MUST 规范化措辞
...
2. 如果 crate 有明显架构/unsafe 约束/feature matrix，创建 `openspec/specs/syscall/design.md`
