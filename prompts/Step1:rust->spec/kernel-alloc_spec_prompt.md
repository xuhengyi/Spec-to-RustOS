# Spec 生成 Prompt: kernel-alloc

## 角色
你是资深 Rust/OS 工程师与规格工程师。

## 任务
为 crate `kernel-alloc` 生成 OpenSpec capability specs。

## 输入文件
请阅读以下文件（在项目根目录下）：
- `kernel-alloc/Cargo.toml`
- `kernel-alloc/src/**/*.rs`
- `kernel-alloc/build.rs`（如果存在）
- `kernel-alloc/README.md`（如果存在）

## 输出要求

1. 创建 `openspec/specs/kernel-alloc/spec.md`，包含：
   - 使用 SHALL/MUST 规范化措辞
...
2. 如果 crate 有明显架构/unsafe 约束/feature matrix，创建 `openspec/specs/kernel-alloc/design.md`
