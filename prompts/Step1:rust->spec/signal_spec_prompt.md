# Spec 生成 Prompt: signal

## 角色
你是资深 Rust/OS 工程师与规格工程师。

## 任务
为 crate `signal` 生成 OpenSpec capability specs。

## 输入文件
请阅读以下文件（在项目根目录下）：
- `signal/Cargo.toml`
- `signal/src/**/*.rs`
- `signal/build.rs`（如果存在）
- `signal/README.md`（如果存在）

## 依赖关系
该 crate 依赖以下 workspace 内 crate：
kernel-context signal-defs 

请将这些依赖的所需符号/语义写成前置条件（Preconditions）。

## 输出要求

1. 创建 `openspec/specs/signal/spec.md`，包含：
   - 使用 SHALL/MUST 规范化措辞
...
2. 如果 crate 有明显架构/unsafe 约束/feature matrix，创建 `openspec/specs/signal/design.md`
