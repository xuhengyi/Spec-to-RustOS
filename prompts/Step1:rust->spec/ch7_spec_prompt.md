...
## 任务
为 crate `ch7` 生成 OpenSpec capability specs。
...
linker console kernel-context kernel-alloc kernel-vm syscall task-manage easy-fs signal signal-impl linker 

请将这些依赖的所需符号/语义写成前置条件（Preconditions）。

## 输出要求

1. 创建 `openspec/specs/ch7/spec.md`，包含：
   - 使用 SHALL/MUST 规范化措辞
...
2. 如果 crate 有明显架构/unsafe 约束/feature matrix，创建 `openspec/specs/ch7/design.md`
