...
## 任务
为 crate `ch3` 生成 OpenSpec capability specs。
...
linker console kernel-context syscall linker 

请将这些依赖的所需符号/语义写成前置条件（Preconditions）。

## 输出要求

1. 创建 `openspec/specs/ch3/spec.md`，包含：
   - 使用 SHALL/MUST 规范化措辞
...
2. 如果 crate 有明显架构/unsafe 约束/feature matrix，创建 `openspec/specs/ch3/design.md`
