# 实现生成 Prompt: easy-fs Step 5 - vfs-inode

## 角色
你是 Rust OS crate 的实现者。

## 任务
从 OpenSpec spec 实现 easy-fs 的 **vfs-inode** 模块（第 5 步，共 5 步，最终步骤）。

## 输入文件
- `openspec/specs/vfs-inode/spec.md`
- `openspec/specs/vfs-inode/design.md`（如有）
- `easy-fs/Cargo.toml`
- `easy-fs/src/`（Step 1-4 已生成的代码）

## 访问规则（重要！）

### 允许的访问
1. **当前模块的 spec**: 必须阅读 `openspec/specs/vfs-inode/spec.md` 和 design.md（如有）
2. **直接依赖的 spec（可选，需记录）**: 如果当前模块的 spec 不足以理解接口，可以阅读直接依赖的 spec：
   - `openspec/specs/block-device/spec.md`
   - `openspec/specs/block-cache/spec.md`
   - `openspec/specs/disk-layout/spec.md`
   - `openspec/specs/fs-manager/spec.md`
...
## 约束
- **仅实现当前模块**: 只修改 `easy-fs/` 目录下的文件
- **优先使用当前模块的 spec**: 首先尝试仅通过当前模块的 spec 实现
...
## 输出
只提交该 crate 目录下必要的 Rust 源码/配置。

---

## 实现日志

**必须维护实现日志**: `implementation_logs/easy-fs_step5.log`
