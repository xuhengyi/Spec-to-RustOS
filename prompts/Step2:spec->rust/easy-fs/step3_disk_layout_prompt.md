# 实现生成 Prompt: easy-fs Step 3 - disk-layout

## 角色
你是 Rust OS crate 的实现者。

## 任务
从 OpenSpec spec 实现 easy-fs 的 **disk-layout** 模块（第 3 步，共 5 步）。

## 输入文件
- `openspec/specs/disk-layout/spec.md`
- `openspec/specs/disk-layout/design.md`（如有）
- `easy-fs/Cargo.toml`
- `easy-fs/src/`（Step 1-2 已生成的代码）

## 访问规则（重要！）

### 允许的访问
1. **当前模块的 spec**: 必须阅读 `openspec/specs/disk-layout/spec.md` 和 design.md（如有）
...
**必须维护实现日志**: `implementation_logs/easy-fs_step3.log`
