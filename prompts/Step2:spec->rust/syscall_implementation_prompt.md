...
## 任务
从 OpenSpec spec 实现 crate `syscall`。

## 输入文件
- `openspec/specs/syscall/spec.md`
- `openspec/specs/syscall/design.md`
- `syscall/Cargo.toml`

## 访问规则（重要！）

### 允许的访问
1. **当前 crate 的 spec**: 必须阅读 `openspec/specs/syscall/spec.md` 和 design.md（如有）
2. **直接依赖的 spec（可选，需记录）**: 如果当前 crate 的 spec 不足以理解接口，可以阅读直接依赖的 spec：
   - `openspec/specs/signal-defs/spec.md` (crate: `signal-defs`)
...
## 约束
- **仅实现当前 crate**: 只修改 `syscall/` 目录下的文件
...
## 工作流程
1. 阅读 `openspec/specs/syscall/spec.md` 和 design.md（如有）
2. 尝试仅基于当前 crate 的 spec 实现
3. 如果当前 spec 不足以理解接口，可以阅读直接依赖的 specs（**必须在日志中说明原因**）
4. 实现 crate（创建或修改 `syscall/src/lib.rs` 或 `syscall/src/main.rs`）
5. 运行 gate 验证：cd 到 `syscall` 目录，执行 `cargo check` 和 `cargo test`
6. 如果测试失败且无法通过 spec 解决，可以访问已生成的代码（**必须在日志中详细说明**）
7. 更新实现日志：`implementation_logs/syscall_implementation.log`
...
**必须维护实现日志**: `implementation_logs/syscall_implementation.log`
