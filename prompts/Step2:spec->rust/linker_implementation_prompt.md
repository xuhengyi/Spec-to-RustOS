...
## 任务
从 OpenSpec spec 实现 crate `linker`。

## 输入文件
- `openspec/specs/linker/spec.md`
- `openspec/specs/linker/design.md`
- `linker/Cargo.toml`

## 访问规则（重要！）

### 允许的访问
1. **当前 crate 的 spec**: 必须阅读 `openspec/specs/linker/spec.md` 和 design.md（如有）
...
## 工作流程
1. 阅读 `openspec/specs/linker/spec.md` 和 design.md（如有）
2. 尝试仅基于当前 crate 的 spec 实现
3. 如果当前 spec 不足以理解接口，可以阅读直接依赖的 specs（**必须在日志中说明原因**）
4. 实现 crate（创建或修改 `linker/src/lib.rs` 或 `linker/src/main.rs`）
5. 运行 gate 验证：cd 到 `linker` 目录，执行 `cargo check` 和 `cargo test`
6. 如果测试失败且无法通过 spec 解决，可以访问已生成的代码（**必须在日志中详细说明**）
7. 更新实现日志：`implementation_logs/linker_implementation.log`
...
**必须维护实现日志**: `implementation_logs/linker_implementation.log`
