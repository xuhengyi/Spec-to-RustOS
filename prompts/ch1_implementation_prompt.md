...
## 任务
从 OpenSpec spec 实现 crate `ch1`。

## 输入文件
- `openspec/specs/ch1/spec.md`
- `openspec/specs/ch1/design.md`
- `ch1/Cargo.toml`

## 访问规则（重要！）

### 允许的访问
1. **当前 crate 的 spec**: 必须阅读 `openspec/specs/ch1/spec.md` 和 design.md（如有）
...
## 工作流程
1. 阅读 `openspec/specs/ch1/spec.md` 和 design.md（如有）
2. 尝试仅基于当前 crate 的 spec 实现
3. 如果当前 spec 不足以理解接口，可以阅读直接依赖的 specs（**必须在日志中说明原因**）
4. 实现 crate（创建或修改 `ch1/src/lib.rs` 或 `ch1/src/main.rs`）
...
8. 更新实现日志：`implementation_logs/ch1_implementation.log`
...
**必须维护实现日志**: `implementation_logs/ch1_implementation.log`
