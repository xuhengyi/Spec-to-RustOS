# 实现生成 Prompt: easy-fs Step 2 - block-cache

## 角色
你是 Rust OS crate 的实现者。

## 任务
从 OpenSpec spec 实现 easy-fs 的 **block-cache** 模块（第 2 步，共 5 步）。

## 输入文件
- `openspec/specs/block-cache/spec.md`
- `openspec/specs/block-cache/design.md`（如有）
- `easy-fs/Cargo.toml`
- `easy-fs/src/`（Step 1 已生成的代码）

## 访问规则（重要！）

### 允许的访问
1. **当前模块的 spec**: 必须阅读 `openspec/specs/block-cache/spec.md` 和 design.md（如有）
...
**必须维护实现日志**: `implementation_logs/easy-fs_step2.log`
