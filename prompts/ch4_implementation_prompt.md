...
## 任务
从 OpenSpec spec 实现 crate `ch4`。

## 输入文件
- `openspec/specs/ch4/spec.md`
- `openspec/specs/ch4/design.md`
- `ch4/Cargo.toml`

## 访问规则（重要！）

### 允许的访问
1. **当前 crate 的 spec**: 必须阅读 `openspec/specs/ch4/spec.md` 和 design.md（如有）
...
3. **需要验证的 crate 代码（集成测试阶段允许）**: 以下 crate 需要在当前章节的集成测试阶段才能验证正确性。
   ...
   - `kernel-alloc/` (crate: `kernel-alloc`): 允许访问和修改 `kernel-alloc/src/` 下的源代码文件
   - `kernel-vm/` (crate: `kernel-vm`): 允许访问和修改 `kernel-vm/src/` 下的源代码文件
...
- **集成测试验证**: 以下 crate 需要在当前章节的集成测试阶段才能验证正确性，如果发现问题可以修改：
  - `kernel-alloc/` (crate: `kernel-alloc`)
  - `kernel-vm/` (crate: `kernel-vm`)
- **仅实现当前 crate**: 只修改 `ch4/` 目录下的文件
...
## 工作流程
1. 阅读 `openspec/specs/ch4/spec.md` 和 design.md（如有）
2. 尝试仅基于当前 crate 的 spec 实现
3. 如果当前 spec 不足以理解接口，可以阅读直接依赖的 specs（**必须在日志中说明原因**）
4. 实现 crate（创建或修改 `ch4/src/lib.rs` 或 `ch4/src/main.rs`）
5. 运行 gate 验证：`cargo qemu --ch 4`
6. **验证相关 crate 的正确性**: 如果集成测试发现问题，检查并修复以下 crate 的实现：
   - `kernel-alloc/` (crate: `kernel-alloc`): 
     * 访问 `kernel-alloc/src/` 下的源代码文件，检查实现是否正确
     * 如果发现问题，修改代码以修复集成测试问题
     * **必须在日志中记录**: 发现的问题、修改的文件、修改原因和解决方案
   - `kernel-vm/` (crate: `kernel-vm`): 
     * 访问 `kernel-vm/src/` 下的源代码文件，检查实现是否正确
     * 如果发现问题，修改代码以修复集成测试问题
     * **必须在日志中记录**: 发现的问题、修改的文件、修改原因和解决方案
7. **验证输出**：访问 `user/src/bin/` 目录下的测试程序代码，检查 `cargo qemu --ch 4` 的输出是否符合预期
...
9. 更新实现日志：`implementation_logs/ch4_implementation.log`
...
## 输出验证
验证 `cargo qemu --ch 4` 的输出是否符合预期：
1. 查看 `user/cases.toml` 中 `[ch4]` 部分，了解需要运行的测试用例列表
2. 访问 `user/src/bin/` 目录下对应的测试程序源代码（如 `00hello_world.rs`、`02power.rs` 等）
3. 理解每个测试程序的预期输出行为
4. 运行 `cargo qemu --ch 4` 并检查实际输出是否与预期一致
...
**必须维护实现日志**: `implementation_logs/ch4_implementation.log`
