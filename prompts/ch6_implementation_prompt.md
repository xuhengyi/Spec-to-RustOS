...
### 允许的访问
1. **当前 crate 的 spec**: 必须阅读 `openspec/specs/ch6/spec.md` 和 design.md（如有）
...
## 工作流程
1. 阅读 `openspec/specs/ch6/spec.md` 和 design.md（如有）
2. 尝试仅基于当前 crate 的 spec 实现
3. 如果当前 spec 不足以理解接口，可以阅读直接依赖的 specs（**必须在日志中说明原因**）
4. 实现 crate（创建或修改 `ch6/src/lib.rs` 或 `ch6/src/main.rs`）
5. 运行 gate 验证：`cargo qemu --ch 6`
6. **验证输出**：访问 `user/src/bin/` 目录下的测试程序代码，检查 `cargo qemu --ch 6` 的输出是否符合预期
...
8. 更新实现日志：`implementation_logs/ch6_implementation.log`
...
## 输出验证
验证 `cargo qemu --ch 6` 的输出是否符合预期：
1. 查看 `user/cases.toml` 中 `[ch6]` 部分，了解需要运行的测试用例列表
2. 访问 `user/src/bin/` 目录下对应的测试程序源代码（如 `00hello_world.rs`、`02power.rs` 等）
3. 理解每个测试程序的预期输出行为
4. 运行 `cargo qemu --ch 6` 并检查实际输出是否与预期一致
...
**必须维护实现日志**: `implementation_logs/ch6_implementation.log`
