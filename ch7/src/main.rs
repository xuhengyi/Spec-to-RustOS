...
.globl __ch7_portal_code
.globl __ch7_portal_trap
.globl __ch7_portal_code_end
.align 4
__ch7_portal_code:
    sd   a1, 8(a0)
    ld   a1, 16(a0)
...
pub static mut PROCESSOR: Option<PManager<Process, ProcManager>> = None;
...
impl syscall::Signal for SyscallContext {
    fn kill(&self, _caller: Caller, pid: isize, signum: u8) -> isize {
        ...
        let Some(target) = processor.get_task(target_pid) else {
            return -1;
        };
    ...
    fn sigreturn(&self, _caller: Caller) -> isize {
        ...
        if proc.signal.sig_return(&mut proc.context.context) {
            0
        } else {
            -1
        }
...
extern "C" fn rust_main() -> ! {
    ...
    unsafe {
        extern "C" {
            fn __ch7_portal_code();
            fn __ch7_portal_code_end();
        }
        let src = __ch7_portal_code as *const u8;
        let len = (__ch7_portal_code_end as usize).saturating_sub(__ch7_portal_code as usize);
        ...
        match trap_cause.cause() {
            scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                ...
                if next_exit.is_none() {
                    match proc.signal.handle_signals(&mut proc.context.context) {
                ...
                if let Some(code) = next_exit {
                    let processor = unsafe { PROCESSOR.as_mut().unwrap() };
                    processor.make_current_exited(code);
